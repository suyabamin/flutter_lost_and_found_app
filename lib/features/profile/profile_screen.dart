        ...
        child: Column(
          children: [
            ...
            StreamBuilder<List<RatingModel>>(
              stream: ref.watch(firestoreServiceProvider).streamRatingsForUser(user?.uid ?? 'guest'),
              builder: (context, ratingsSnap) {
                final ratings = ratingsSnap.data ?? [];
                final ratingCount = ratings.length;
                double calcAvg = 5.0;
                if (ratings.isNotEmpty) {
                  final sum = ratings.fold<double>(0.0, (prev, r) => prev + r.rating);
                  calcAvg = double.parse((sum / ratings.length).toStringAsFixed(1));
                }
                final displayAvg = ratings.isNotEmpty ? calcAvg : (user?.averageRating ?? 5.0);
                final displayCount = ratings.isNotEmpty ? ratingCount : (user?.totalReviews ?? 0);

                return StreamBuilder(
                  stream: ref.watch(firestoreServiceProvider).streamWallet(user?.uid ?? 'guest'),
                  builder: (context, walletSnap) {
                    final wallet = walletSnap.data;
                    final totalEarned = wallet?.totalEarned.toInt() ?? 0;

                    return StreamBuilder(
                      stream: ref.watch(firestoreServiceProvider).streamUserHistory(user?.uid ?? 'guest'),
                      builder: (context, historySnap) {
                        final historyList = historySnap.data ?? [];
                        final recCount = user?.completedRecoveries ?? historyList.length;
                              ...
                              children: [
                                Expanded(
                                  child: GlassContainer(
                                    ...
                                    child: Column(
                                      children: [
                                        Text('$recCount', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: AppColors.primary)),
                                        const SizedBox(height: 2),
                                        const Text('Recoveries', style: TextStyle(fontSize: 11, color: AppColors.outline)),
                                      ],
                                    ),
                                  ),
                                ),
                                ...
                                Expanded(
                                  child: GlassContainer(
                                    ...
                                    child: Column(
                                      children: [
                                        Text('$retCount', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: AppColors.secondary)),
                                        const SizedBox(height: 2),
                                        const Text('Returns', style: TextStyle(fontSize: 11, color: AppColors.outline)),
                                      ],
                                    ),
                                  ),
                                ),
                                ...
                                Expanded(
                                  child: GlassContainer(
                                    ...
                                    child: Column(
                                      children: [
                                        Text('৳ $totalEarned', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.green)),
                                        const SizedBox(height: 2),
                                        const Text('Earned', style: TextStyle(fontSize: 11, color: AppColors.outline)),
                                      ],
                                    ),
                                  ),
                                ),
                              ...
                              children: [
                                      ...
                                      children: [
                                        Row(
                                          ...
                                          children: [
                                            ...
                                            Text('$displayAvg', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                                          ],
                                        ),
                                        const SizedBox(height: 2),
                                        Text('$displayCount Reviews', style: const TextStyle(fontSize: 11, color: AppColors.outline)),
                                      ],
                                    ),
                                  ),
                                ),
                                ...
                                Expanded(
                                  child: GlassContainer(
                                    ...
                                    child: Column(
                                      children: [
                                        Text('$trustScore%', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.teal)),
                                        const SizedBox(height: 2),
                                        const Text('Trust Score', style: TextStyle(fontSize: 11, color: AppColors.outline)),
                                      ],
                                    ),
                                  ),
                                ),
                              ...
                              child: Column(
                                ...
                                children: [
                                    ...
                                    children: List.generate(5, (index) {
                                      ...
                                      return Icon(
                                        starVal <= displayAvg.floor()
                                            ? Icons.star_rounded
                                            : (starVal - displayAvg <= 0.5 ? Icons.star_half_rounded : Icons.star_outline_rounded),
                                        color: Colors.amber,
                                        size: 20,
                                      );
                                    }),
                                  ...
                                  if (ratings.isNotEmpty) ...[
                                    ...
                                    const Text('Recent Ratings & Reviews:', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: AppColors.primary)),
                                    const SizedBox(height: 8),
                                    ListView.separated(
                                      shrinkWrap: true,
                                          ...
                                          child: Column(
                                            ...
                                            children: [
                                              ...
                                              if (r.review.isNotEmpty) ...[
                                                const SizedBox(height: 4),
                                                Text('"${r.review}"', style: const TextStyle(fontSize: 12, fontStyle: FontStyle.italic)),
                                              ],
                                            ],
                                          ),
                                        );
              ...
              child: Column(
                children: [
                  ...
                  ListTile(
                    ...
                    subtitle: const Text('Reward earnings & transaction history'),
                    trailing: const Icon(Icons.chevron_right_rounded),
